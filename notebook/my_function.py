...
def process_nari_phenology(base, dates, out_fig):
    """Courbes NARI avec écarts-types."""
    mask = rw.load_img_as_array(os.path.join(base, 'sample_strata.tif'))
...
def prepare_classification_data(base, ref, shp, spl_rst, bands, extra=[]):
    """Extraction Données."""
    # Groupes
    ids_rst = os.path.join(base, 'PI_ids_rasterized.tif')
...
def optimize_random_forest(X, Y, G):
    """GridSearch."""
...
def evaluate_model(model, X, Y, G, names, out_cm, out_qual, nb_iter=5):
    ...
    # 2. Matrice Moyenne (Style image_be6674.jpg)
    ...
    if out_cm: plt.savefig(out_cm, bbox_inches='tight')
    plt.show()

    # 3. Qualité (Style image_bebd2b.jpg)
    ...
    plt.tight_layout()
    if out_qual: plt.savefig(out_qual, bbox_inches='tight')
    plt.show()
    return model
...
def produce_final_map(model, base, bands, out):
    """Inférence Finale."""
    # 1. Stack
    l_arr = []
    for b in bands:
    ...
    nari = rw.load_img_as_array('results/ARI_serie_temp.tif')
